...
def sorteer_op_maandloon():
    ...
    print("\nGesorteerde personeelslijst op maandloon (hoog naar laag):")
...
# Functie om de gesorteerde lijst op te vragen
def vraag_sorteer_en_toon():
    while True:
        keuze = input("Wil je de personeelslijst sorteren op maandloon? (ja/nee): ").strip().lower()
        if keuze == 'ja':
            sorteer_op_maandloon()
            break
        elif keuze == 'nee':
            print("Sorteren is overgeslagen.")
            break
        else:
